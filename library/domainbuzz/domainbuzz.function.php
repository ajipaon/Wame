...
function changeTitle($title,$description='', $img=''){
	...
	$img = ($img) ? $img : base_url('assets/img/logo.jpg');
...
function grabeDomainByGoolgeSearch($key, $u){
$data = array();
$i = 0;
$va = file_get_html('https://www.google.com/search?q='.urlencode($key).'&source=lnms&filter=0&start='.$u);
foreach($va->find('a') as $element){
   if(strpos($element,"q=http") !== false){
    $parse =  parse_url($element->href);
   //var_dump($parse['query']);
    $str = substr_replace($parse['query'],"",0,2);
    $dar = parse_url($str);
     $out = $dar['host']; // tanpa https
      $data[$i] = $out;
      $i++;
    }
  }
...
function cleanDomainUrl($host){
  $myhost = strtolower(trim($host));
  $count = substr_count($myhost, '.');
  if($count === 2){
    if(strlen(explode('.', $myhost)[1]) > 3) $myhost = explode('.', $myhost, 2)[1];
  } else if($count > 2){
    $myhost = cleanDomainUrl(explode('.', $myhost, 2)[1]);
  }
  return $myhost;
}

function cleanString_Domain($a){
    
    $ffs = explode(":", $a);
    return $ffs[1];
}

function cleanString_Update_creation_Expired($update){
...
function admin1($user) {
	if($user == 'ajisetiawan883@gmail.com') {
	return true;
	} else {
		return false;
	}
	...
	if(strtotime($today) >= strtotime($expiry)) {
		if(isset($_COOKIE['refresh_token'])) {
			...
			if(!empty($refresh->access_token)) {
				...
				setcookie("token_expired_in_time", date('Y-m-d H:i:s', strtotime('+1 hour')), time()+3600*24*30, '/', BASE_DOMAIN, TRUE, TRUE);
...
function check_file_access($file_id) {
	global $_user;
	global $BoxClass; global $app;
	$get_file = $BoxClass->get_file($file_id);
